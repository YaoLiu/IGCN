...
class Manager():
    def __init__(self, model, args):
        ...
        print(self.device)
        if args.load:
            model.load_state_dict({k.replace('module.',''):v for k,v in torch.load(args.load).items()})
            # model.load_state_dict(torch.load(args.load))
        # self.model = torch.nn.DataParallel(model, device_ids=[5,7])
    ...
    def train(self, train_data, test_data):
        ...
        for epoch in range(self.epoch):
            ...
            for i, (points, gt) in enumerate(train_data):
                # print('-- model-' + str(i) + ' : '+ time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
                ...
                if (i + 1) % self.record_interval == 0:
                    print(' epoch {:3d} step {:5d} | avg loss: {:.5f} | avg acc: {:.5f}'.format(epoch +1, i+1, train_loss/(i + 1), train_label.get_acc()))
                    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))

                if (i + 1) % (self.record_interval * 10) == 0:
                    self.record(' epoch {:3d} step {:5d} | avg loss: {:.5f} | avg acc: {:.5f}'.format(epoch +1, i+1, train_loss/(i + 1), train_label.get_acc()))
                    self.record(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
