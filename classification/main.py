...
def main():
    ...
    parser.add_argument('-rate', type= float, default= 1.0, help= 'data rate')
    ...
    if args.mode == "train":
        print('Trianing ...')
        train_data = ModelNet_pointcloud(args.dataset, 'train', transform= transform, rate=args.rate)
        ...
        test_loader = DataLoader(test_data, shuffle= False, batch_size= args.bs, drop_last=True)
    
        manager.train(train_loader, test_loader)

    else:
        print('Testing ...')
        test_data = ModelNet_pointcloud(args.dataset, 'test', transform= transform, rate=args.rate)
        test_loader = DataLoader(test_data, shuffle= False, batch_size= args.bs, drop_last=True)
