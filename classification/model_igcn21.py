...
class GCN3D(nn.Module):
    def __init__(self, support_num: int, neighbor_num: int):
        ...
        self.conv_6 = gcn3d.Conv_layer(64, 32, support_num= support_num)
        # self.conv_7 = gcn3d.Conv_layer(224, 32, support_num= support_num)
        self.conv_8 = gcn3d.Conv_layer(96, 1024, support_num= support_num)
    ...
    def forward(self,  vertices: "(bs, vertice_num, 3)"):
        ...
        neighbor_index = gcn3d.query_ball_point(0.3, self.neighbor_num, vertices, vertices)
        ...
        vertices, fm_1 = self.pool_1(vertices, fm_1, 0.3)
        # neighbor_index = gcn3d.get_neighbor_index(vertices, self.neighbor_num)
        neighbor_index = gcn3d.query_ball_point(0.5, self.neighbor_num, vertices, vertices)
        ...
        # vertices, fm_5 = self.pool_2(vertices, fm_5, 0.39)
        # # neighbor_index = gcn3d.get_neighbor_index(vertices, self.neighbor_num)
        # neighbor_index = gcn3d.query_ball_point(0.63, self.neighbor_num, vertices, vertices)

        fm_6 = self.conv_6(neighbor_index, vertices, fm_1)
        fm_6 = F.relu(fm_6, inplace= True) 
        fm_6 = torch.cat((fm_1, fm_6), 2)

        # fm_7 = self.conv_7(neighbor_index, vertices, fm_6)
        # fm_7 = F.relu(fm_7, inplace= True) 
        # fm_7 = torch.cat((fm_6, fm_7), 2)
        
        fm_8 = self.conv_8(neighbor_index, vertices, fm_6)
