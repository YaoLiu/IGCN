"""
@Author: Yao LIU
@Contact: yao.liu3@unsw.edu.au
@Time: 2021/07/14
最终版igcn post融合-pre融合
"""
...
class Conv_surface(nn.Module):
    def __init__(self, in_channel, out_channel, support_num):
        ...
        # parameters:
        self.directions = nn.Parameter(torch.FloatTensor(4, in_channel)) # 4: support_num
        self.initialize()
        

    def initialize(self):
        stdv = math.sqrt(2. / self.in_channel)
    ...
    def forward(self, 
        ...
        feature_support = feature_support * theta
        feature_support = feature_support.contiguous().view(-1, self.in_channel) # (bs * vertice_num * neighbor_num, 4)
        feature_support = self.nn(feature_support) # (bs * vertice_num * neighbor_num, out_channel)
        feature_support = feature_support.contiguous().view(bs, vertice_num, neighbor_num, -1) # (bs, vertice_num, neighbor_num, out_channel)

        # activation_support = feature_support * theta # (bs, vertice_num, neighbor_num, out_channel)
        activation_support = torch.max(feature_support, dim= 2)[0] # (bs, vertice_num, out_channel)  
...
class Conv_layer(nn.Module):
    def __init__(self, in_channel, out_channel, support_num):
        ...
        self.out_channel = out_channel # 32, 32
        self.support_num = support_num # fiexd 4
    ...
    def forward(self, 
        ...
        theta = neighbor_direction_norm_weight @ support_direction_norm_weight + support_direction_norm[0] # (bs, vertice_num, neighbor_num, in_channel)

        # activation_support = feature_support * theta # (bs, vertice_num, neighbor_num, out_channel)

        feature_support = indexing_neighbor(feature_map, neighbor_index) # (bs, vertice_num, neighbor_num, in_channel)
        feature_support = feature_support * theta
        feature_support = feature_support.contiguous().view(-1, self.in_channel) # (bs * vertice_num * neighbor_num, in_channel)
        feature_support = self.nn(feature_support) # (bs * vertice_num * neighbor_num, out_channel)
        feature_support = feature_support.contiguous().view(bs, vertice_num, neighbor_num, -1) # (bs, vertice_num, neighbor_num, out_channel)
